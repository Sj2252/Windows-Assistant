...
APPS = {
    "notepad": {"command": "notepad", "window_title": "Notepad", "type": "system", "close_search": "Notepad"},#1
    "calculator": {"command": "calc", "window_title": "Calculator", "type": "system", "close_search": "Calculator"},#2
    "chrome": {"command": "chrome", "window_title": "Google Chrome", "type": "system", "close_search": "Chrome"},#3
    "google chrome": {"command": "chrome", "window_title": "Google Chrome", "type": "system", "close_search": "Chrome"},#4
    "excel": {"type": "office", "office_app": "Excel.Application", "close_search": "Excel"},#5
    "word": {"type": "office", "office_app": "Word.Application", "close_search": "Word"},#6
    "powerpoint": {"type": "office", "office_app": "PowerPoint.Application", "close_search": "PowerPoint"},#7
    "outlook": {"type": "office", "office_app": "Outlook.Application", "close_search": "Outlook"},#8
    "paint": {"command": "mspaint", "window_title": "Paint", "type": "system", "close_search": "Paint"},#9
    "vlc": {"command": "vlc", "window_title": "VLC media player", "type": "system", "close_search": "VLC"},#10
    "explorer": {"command": "explorer", "window_title": "File Explorer", "type": "system", "close_search": "Explorer"},#11
    "cmd": {"command": "cmd.exe", "window_title": "Command Prompt", "type": "system", "close_search": "Command Prompt"},#12
    "Microsoft ed" or "microsoft edge": {"command": "msedge", "window_title": "Microsoft Edge", "type": "system", "close_search": "Microsoft Edge"}#13
}
...
def close_app(substring):
    """Close applications whose window title contains the substring"""
    closed = [False]
    def callback(hwnd, closed):
        title = win32gui.GetWindowText(hwnd)
        if substring.lower() in title.lower():
...
def close_app_by_name(app_name):
    ...
    if app_lower in APPS:
        app_config = APPS[app_lower]
        if app_lower == "microsoft edge":
            # Special handling for Microsoft Edge - kill the process
            try:
            ...
            close_search = app_config.get("close_search", app_config.get("window_title", app_lower))
            if close_app(close_search):
                speak(f"Closed {app_lower}")
                return True
            else:
                speak(f"{app_lower} is not running")
